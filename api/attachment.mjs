...
export default async function handler(request) {
  ...
  try {
    ...
    if (raw.length > 7500000) return json({ error: 'File is too large; limit is 5 MB' }, 413);
    ...
    if (!bytes.length || bytes.length > 5 * 1024 * 1024) return json({ error: 'File must be smaller than 5 MB' }, 413);
