...
(() => {
  ...
  function renderJob() {
    ...
    const folderControl=folderUrl?`<a class="hfo-btn" href="${escape(folderUrl)}" target="_blank" rel="noopener noreferrer">Open GitHub folder</a>${isOwner()?'<button class="hfo-btn" data-sync-job-folder>Sync Service folders</button>':''}<small class="hfo-muted">Folder name is kept from creation if Vessel, Job No. or ETA is edited.</small>`:isOwner()?'<button class="hfo-btn" data-sync-job-folder>Create GitHub folder</button>':'<small class="hfo-muted">GitHub folder not created</small>';
