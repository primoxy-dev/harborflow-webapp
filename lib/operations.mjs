...
// Anonymous Operations is a separate, deliberately narrow read model. Never
// return the stored JSON blob: free-text fields can contain person data.
export function publicJob(row) {
  const { jobNo, vessel, port, principal, eta, etd, status } = row.data;
  return { id: row.id, data: { jobNo, vessel, port, principal, eta, etd, status } };
}
export function publicService(row) {
  const { type, status, plannedStart, plannedEnd, actualStart, actualEnd } = row.data;
  return { id: row.id, job_id: row.job_id, seq: row.seq, data: { type, status, plannedStart, plannedEnd, actualStart, actualEnd } };
}
