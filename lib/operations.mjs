...
export function publicJob(row) {
  const { vessel, port, eta, status } = row.data;
  return { id: row.id, data: { vessel, port, eta, status } };
}
export function publicService(row) {
  const { type, status } = row.data;
  return { id: row.id, job_id: row.job_id, seq: row.seq, data: { type, status } };
}
