...
export async function gitData(method, path, body) {
  const response = await fetch(`${githubApi}/repos/${repo}/git/${path}`, {
    method,
    headers: { ...headers(process.env.GITHUB_DOCUMENTS_TOKEN), ...(body ? { 'Content-Type': 'application/json' } : {}) },
    body: body ? JSON.stringify(body) : undefined
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    const error = Error(`GitHub Git ${method} failed (${response.status}): ${data.message || 'Unknown error'}`);
